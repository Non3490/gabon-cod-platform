...
async function main() {
  ...
  for (const sellerData of sellersData) {
    ...
    sellers.push({ id: sellerId, name: sellerData.name, email: sellerData.email })
  }

  // Create Products for each seller
  console.log('📦 Creating seller products and stock...')
  const stockQtys: any = { 'Hervé': 60, 'Chancelle': 40, 'Patrick': 90, 'Laure': 30 }
  ...
  // First pass: create all products and get their IDs
  for (const seller of sellers) {
    const product = products.find(p => p.name === seller.name)
    if (!product) continue
    ...
    if (existingSellerProduct) {
      productBySeller[seller.name] = existingSellerProduct
    } else {
      const newSellerProduct = await prisma.product.create({
        data: {
          sellerId: seller.id,
      ...
      productBySeller[seller.name] = newSellerProduct.id
    }
  }
  ...
  for (const seller of sellers) {
    ...
    const product = productBySeller[seller.name]
    if (!product) continue
